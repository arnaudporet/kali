...
module lib
    ...
    subroutine report_A_set(A_set,setting,V,bool,dec)
        ...
        report=report//"Found attractors: "//int2char(size(A_set))//new_line("a")//"    points: "//int2char(n_point)//new_line("a")//"    cycles: "//int2char(n_cycle)
    ...
    subroutine report_B_therap(B_therap,V,bool,dec,A_physio,a_patho_set,r_min,r_max)
        ...
        do i1=1,size(barrel)-1
            report=report//"    "//int2char(i1+r_min-1)//"-bullets: "//int2char(barrel(i1)+r_min-1)//new_line("a")
        end do
        report=report//"    "//int2char(size(barrel)+r_min-1)//"-bullets: "//int2char(barrel(size(barrel))+r_min-1)
        write (unit=*,fmt="(a)") new_line("a")//report//new_line("a")//new_line("a")//"Save?"//new_line("a")//"    [0] no"//new_line("a")//"    [1] yes"
        read (unit=*,fmt=*) save_
        if (save_==1) then
