...
program example_network
    ...
    function f_physio(x,k) result(y)
        ...
        y(1,1)=x(1,k)!CycD
        y(2,1)=1.0-max(x(1,k),x(7,k),min(max(min(x(3,k),1.0-x(2,k)),x(4,k)),1.0-x(5,k)))!Rb
        ...
        y(6,1)=1.0-max(max(x(7,k),min(x(4,k),1.0-x(5,k))),min(max(x(7,k),min(x(4,k),1.0-x(5,k))),x(6,k),max(x(4,k),x(7,k))))!UbcH10
        y(7,1)=1.0-max(x(7,k),max(x(7,k),min(x(4,k),1.0-x(5,k))))!CycB
