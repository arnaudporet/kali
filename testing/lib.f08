...
module lib
    ...
    !#########################    compute_B_therap    #########################!
    !##########################################################################!
    function compute_B_therap(f,D,r_min,r_max,max_targ,max_moda,n_node,value,A_physio,A_patho) result(B_therap)
        ...
        deallocate(A_test,C_targ,C_moda,b%targ,b%moda)
    ...
    !##########################################################################!
    !#########################    report_B_therap    ##########################! TODO improve how the gain is displayed
    !##########################################################################!
    ...
    !##########################################################################!
    !##########################    sort_B_therap    ###########################! TODO sort according to the gain
    !##########################################################################!
    ...
    subroutine what_to_do(f1,f2,value,size_D,n_node,max_targ,max_moda,V)
        ...
        select case (to_do)
            case (1)
                ...
                select case (setting)
                    case (1)
                        ...
                        deallocate(A_physio)
                    case (2)
                        inquire (file="A_physio.csv",exist=z1)
                        if (.not. z1) then
            ...
            case (2)
                inquire (file="A_patho.csv",exist=z1)
                if (.not. z1) then
            ...
            case (3)
                inquire (file="A_physio.csv",exist=z1)
                inquire (file="A_patho.csv",exist=z2)
                if ((.not. z1) .and. z2) then
                ...
                else if ((.not. z1) .and. (.not. z2)) then
                    ...
                    read (unit=*,fmt=*) r_max
                    B_therap=compute_B_therap(f2,D,r_min,r_max,max_targ,max_moda,n_node,value,A_physio,A_patho)
                    call report_B_therap(B_therap,V,bool)
                    deallocate(A_physio,B_therap,A_patho)
                end if
                deallocate(D)
            case (4)
